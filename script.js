...
function printReceipt(id) {
  ...
  var tendered = Math.max(0, numberValue(s.tenderedAmount, s.paidAmount)), paid = Math.max(0, numberValue(s.paidAmount)), change = Math.max(0, numberValue(s.changeAmount)), returnedChange = getChangeReturned(s.id), remainingChange = getChangeBalance(s), orderStatus = s.orderReceived === true ? "Sudah diterima" : "Belum diterima";
  $('printArea').className = "receipt-print-area";
  $('printArea').innerHTML = '<article class="receipt"><header class="receipt-head"><div class="receipt-brand"><div class="receipt-logo"><img src="logo-matcha.png" alt="Logo El Matcha"></div><div><h1>' + esc(BRAND.name) + '</h1><p>' + esc(BRAND.tagline) + '</p></div></div><div class="receipt-ornament"><i></i><span>✦</span><i></i></div><div class="receipt-kicker">NOTA PENJUALAN</div></header><div class="receipt-meta"><div><span>No. Bill</span><b>' + esc(receiptNo) + '</b></div><div><span>Tanggal</span><b>' + esc(time) + '</b></div></div><div class="receipt-order-status ' + (s.orderReceived === true ? 'received' : 'pending') + '"><span>Status pesanan</span><b>' + orderStatus + '</b></div><table class="receipt-items"><thead><tr><th>Item</th><th>Qty</th><th>Harga</th><th>Subtotal</th></tr></thead><tbody><tr><td><b>' + esc(s.product) + '</b>' + (s.note ? '<small>' + esc(s.note) + '</small>' : '') + '</td><td>' + integerValue(s.qty) + '</td><td>' + rupiah(s.price) + '</td><td>' + rupiah(s.total) + '</td></tr></tbody></table><div class="receipt-total"><span>Total transaksi</span><strong>' + rupiah(s.total) + '</strong></div><div class="receipt-payment"><p><span>Metode pembayaran</span><b>' + esc(salePaymentDisplay(s)) + '</b></p><p><span>Uang diterima</span><b>' + rupiah(tendered) + '</b></p><p><span>Jumlah dibayar</span><b>' + rupiah(paid) + '</b></p><p><span>Total kembalian</span><b>' + rupiah(change) + '</b></p><p><span>Sudah diberikan</span><b>' + rupiah(returnedChange) + '</b></p><p><span>Sisa kembalian</span><b>' + rupiah(remainingChange) + '</b></p>' + (balance > 0 ? '<p class="receipt-debt"><span>Sisa piutang</span><b>' + rupiah(balance) + '</b></p><p><span>Jatuh tempo</span><b>' + esc(s.dueDate || '-') + '</b></p>' : '') + '</div>' + (s.customer ? '<div class="receipt-extra"><span>Pelanggan/Debitur</span><b>' + esc(s.customer) + '</b></div>' : '') + '<footer><div class="receipt-footer-mark">✦</div><b>Terima kasih sudah berkunjung</b><span>Semoga harimu selalu hangat.</span></footer></article>';
  window.print();
}
